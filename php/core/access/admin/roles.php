<?php
  ...
  if (isset($_POST['action'])) {
  	switch($_POST['action']) {
  		case 'saveRole':
  			...
  			foreach ($_POST as $k => $v) {
  				if (substr($k,0,5) == "perm_") {
  					...
  					if ($v == 'X') {
  						$strSQL = sprintf("DELETE FROM access.role_permission rp WHERE rp.role_idrole=%u AND rp.permission_idpermission=%u", $roleID, $permID);
  						pg_query($strSQL);
  						continue;
  					}
  		...
  		break;
  		case 'delRole':
  			$strSQL = sprintf("DELETE FROM access.role r WHERE r.idrole=%u LIMIT 1", $_POST['roleID']);
  			pg_query($strSQL);
  			$strSQL = sprintf("DELETE FROM access.user_role ur WHERE ur.role_idrole=%u", $_POST['roleID']);
  			pg_query($strSQL);
  			$strSQL = sprintf("DELETE FROM access.role_permission rp WHERE rp.role_idrole=%u", $_POST['roleID']);
  			pg_query($strSQL);
        header("location: roles.php");
      break;
    }
...
?>
    	<h2>Manage Role: (<?php echo ($myACL->getRoleNameFromID($_GET['roleID'])); ?>)</h2>
<?php
